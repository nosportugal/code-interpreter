...
export const env = {
    ...
    // Upload Rate Limits
    UPLOAD_LIMIT_WINDOW:
        Number(process.env.UPLOAD_LIMIT_WINDOW) || 5 * 60 * 1000, // 5 minutes
    UPLOAD_MAX_REQUESTS: Number(process.env.UPLOAD_MAX_REQUESTS) || 30, // 30 uploads per 5 minutes
    // Download Rate Limits
    DOWNLOAD_LIMIT_WINDOW:
        Number(process.env.DOWNLOAD_LIMIT_WINDOW) || 60 * 1000, // 1 minute
    ...
    /** TTL on the per-execution replay keys (`exec_state:`, `tool_history:`,
     *  `exec_result:`) and the idle threshold the stale-execution sweep
     *  reaps against. Must exceed the longest gap a client may leave between
     *  continuations of one execution; below `JOB_TIMEOUT` an in-flight
     *  execution can lose its own state mid-run. */
    EXECUTION_STATE_TTL: Math.max(
        Number(process.env.EXECUTION_STATE_TTL) || 600,
        Math.ceil(defaultJobTimeoutMs / 1000),
    ),
    ...
    BRIDGE_DYNAMIC_WORKERS:
        process.env.CODEAPI_BRIDGE_DYNAMIC_WORKERS === 'true',
    ...
    LAMBDA_MICROVM_ALLOW_SHELL:
        process.env.LAMBDA_MICROVM_ALLOW_SHELL === 'true',
