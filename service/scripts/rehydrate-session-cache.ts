...
import { redisKey } from '../src/redis-keys';
...
function parseRecoverySource(
    value: unknown,
    expected: RecoveryScope,
): RecoverySource {
    if (typeof value !== 'object' || value === null || Array.isArray(value)) {
        throw new Error(
            'The first input record must be a recovery source header',
        );
    }
    ...
    try {
        ...
        throw new Error(
            'Recovery source contains invalid environment, region, or namespace',
        );
    }
    if (
        source.type !== 'source' ||
    ...
    if (
        Date.parse(source.query_start_utc) >= Date.parse(source.query_end_utc)
    ) {
        throw new Error(
            'Recovery source query_start_utc must be before query_end_utc',
        );
    }
    return {
        type: 'source',
...
export function parseRecoveryManifest(
    ...
    for (let index = 0; index < lines.length; index += 1) {
        ...
        if (
            previous &&
            previous.expected_session_key !== value.expected_session_key
        ) {
            throw new Error(
                `Input contains conflicting owners for session ${value.session_id}`,
            );
        }
...
async function inspectOwnerRecord(
    ...
    const existing = await store.get(
        redisKey(`session-owner:${record.session_id}`),
    );
...
async function refreshOwnerTtl(
    ...
    if (
        remaining >= 0 &&
        remaining < ownerTtlSeconds &&
        (await store.expire(ownerKey, ownerTtlSeconds)) === 0
    ) {
        return 'vanished';
    }
...
async function ensureOwnerRecord(
    ...
    const ownerKey = redisKey(`session-owner:${record.session_id}`);

    if (existing !== null) {
        const refreshed = await refreshOwnerTtl(
            store,
            ownerKey,
            record,
            ownerTtlSeconds,
    ...
    for (const record of records) {
        try {
            ...
            if (inspection.status === 'conflict') {
                ...
                console.error(
                    `Conflict: ${record.session_id} has a durable owner record for a different owner`,
                );
                continue;
            }

            const cacheKey = redisKey(`session:${record.session_id}`);
            ...
            const settleOwnerRecord = async (
                ...
                if (!options.apply) {
                    const pending = await ownerRepairPending(
                        store,
                        record,
                        ownerTtlSeconds,
                        inspection.existing,
                ...
                const outcome = await ensureOwnerRecord(
                    store,
                    record,
                    ownerTtlSeconds,
                    inspection.existing,
                ...
                if (outcome === 'conflict') {
                    ...
                    // eslint-disable-next-line no-console
                    console.error(
                        `Conflict: ${record.session_id} durable owner record changed during recovery`,
                    );
                    return 'conflicts';
                }
                ...
                // eslint-disable-next-line no-console
                console.error(
                    `Missing: ${record.session_id} durable owner record could not be created`,
                );
                return 'missing';
            };
            ...
            if (current !== null) {
                ...
                // eslint-disable-next-line no-console
                console.error(
                    `Conflict: ${record.session_id} already has a different owner`,
                );
                continue;
            }
            ...
            const racedValue = await store.get(cacheKey);
            if (racedValue === record.expected_session_key) {
                ...
                // eslint-disable-next-line no-console
                console.error(
                    `Missing: ${record.session_id} disappeared during recovery`,
                );
            } else {
                summary.conflicts += 1;
                // eslint-disable-next-line no-console
                console.error(
                    `Conflict: ${record.session_id} changed during recovery`,
                );
            }
        } catch (error) {
...
async function readInput(inputPath?: string): Promise<string> {
    ...
    if (process.stdin.isTTY === true) {
        throw new Error(
            'No manifest input: pipe JSONL on stdin or use --input <path>',
        );
    }
...
export async function main(
    ...
    try {
        ...
        const manifest = parseRecoveryManifest(
            await readInput(options.inputPath),
            options,
        );
        ...
        if (
            ...
            // eslint-disable-next-line no-console
            console.log(
                JSON.stringify(recoveryOutput(options, source, error.summary)),
            );
        }
        // eslint-disable-next-line no-console
